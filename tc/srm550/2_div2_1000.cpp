        ...
        string findOrder(vector <string> grid) {
            ...
            while(1){
                ...
                for(int k=256-1; k>=0; k--){
                    ...
                    if(all_true){
                        could_delete[0].push_back(min_x);
                        could_delete[1].push_back(max_x);
                        could_delete[2].push_back(min_y);
                        could_delete[3].push_back(max_y);
                        chars.push_back(cur);
                ...
                for(int idx=0; idx<chars.size(); idx++){
                    for(int i=could_delete[0][idx]; i<=could_delete[1][idx]; i++){
                        for(int j=could_delete[2][idx]; j<=could_delete[3][idx]; j++){
                            grid[i][j] = '#';
                        }
                    }
                }
