...
double encounter(ant a, ant b){
    ...
    if(a.y < b.y){
        if(b.y - a.y != a.x - b.x){
            if(a.x == b.x){
                if(a.dir == 'E' && b.dir=='W')return (b.y-a.y)*1.0/2;
            }
            return -1;
        }else{
            if((a.dir == 'E' && b.dir == 'N') || (a.dir == 'S' && b.dir == 'W')){
                return b.y - a.y;
            }
        }
    } else{
        if(a.y - b.y != a.x - b.x){
            if(a.y==b.y){
                if(a.dir == 'S' && b.dir == 'N')return (a.x - b.x)*1.0/2;
            }
            return -1;
        }else{
            if((a.dir == 'W' && b.dir == 'N') || (a.dir == 'S' && b.dir == 'E')){
                return a.y - b.y;
            }
        }
    }
...
public:
    int countAnts(vector <int> x, vector <int> y, string direction) {
        ...
        while(1){
            ...
            for(int i = 0; i<sz; i++){
                for(int j = 0; j<sz; j++){
                    if(gone[i]==0 && gone[j] == 0){
                        double dis = encounter(ants[i], ants[j]);
                        if(dis==-1.0)continue;
                        ok = true;
                        if(tmp > dis){
                            px = i;
            ...
            if(ok){            
                int additional = 0;
                for(int i=0; i<sz; i++) {
                    if(gone[i]==0 && i!=px && i!=py){
                        if(encounter(ants[i], ants[px]) == tmp || encounter(ants[i], ants[py])==tmp){
                            gone[i] = 1;
                            additional += 1;
                        }
                    }
