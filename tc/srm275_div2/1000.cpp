...
class GreedyGovernment {
    public:
        double maxAverageCost(vector <string> tolls, int tollHike) {
            ...
            int N = tolls.size();
            while(!Q.empty()){
                piii cur_state = Q.front();
                Q.pop();
                int cur_pos = cur_state.first.first;
                if(cur_pos==N-1){
                    tolls_sum+=cur_state.second;
                    continue;
                }
                int state = cur_state.first.second;
                ...
                for(int i=0; i<n_tolls; i++){
                    if(tolls[cur_pos][i]!='X' && ((state>>i)&1)==0   ){
                        ...
                        piii next_state( pii(i,((1<<i)|state)), cur_state.second+tolls[cur_pos][i]-'0');
                        //pii next_state(i, ((1<<i)|state));
                        mycount[cur_pos][i]++;
                        Q.push(next_state);
                    }
                }
